	  ...
	  _createClass(ExtJsComponentGrid, [{
	    ...
	    value: function edit(callback) {
	      ...
	      try {
	        ...
	        return callback('Failed to get cell element of "' + this.componentType + '" row:#' + rowIndex + '", col:#' + rowIndex + '": ' + e);
	      }

	      new _base.HTMLComponentBase({ htmlElement: htmlElement, driver: this.driver }).click(function (err) {
	        if (err) {
	          return callback('Failed to click on cell element of "' + _this3.componentType + '" row:#' + rowIndex + '", col:#' + rowIndex + '": ' + err);
	        }
	        ...
	        try {
	          ...
	          _this3.driver.getComponent('combobox', '#' + editorElement.id, function (err, editorComponent) {
	            if (err) {
	              return callback('Failed to get editor combobox component of "' + _this3.componentType + '" ' + ('row:#' + rowIndex + '", col:#' + rowIndex + '": ' + err));
	            }
	          ...
	          return callback('Failed to get editor combobox element of "' + _this3.componentType + '" row:#' + rowIndex + '", col:#' + rowIndex + '": ' + e);
	        }
	      });
	    }
	  }, {
